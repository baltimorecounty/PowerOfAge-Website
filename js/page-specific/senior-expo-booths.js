...
seniorExpo.pageSpecific.seniorExpoBooths = (($, undefined) => {
   ...
   */
  const display = (svgFilePath, width, height) => {
    if (!Snap) {
   ...
   */
  const snapLoadHandler = (fragment, floorplan, width, height) => {
    const parentElement = floorplan.append(fragment);
    const booths = parentElement.selectAll('svg > g > g');
  ...
  const highlightAssignedBooths = (boothAssigmentData, booths, callback) => {
    ...
    $.each(booths, (index, snapElement) => {
      const boothAssignmentIndex = assignedBoothIds.indexOf(snapElement.node.id);

      if (boothAssignmentIndex !== -1) {
        const $snapElementNode = $(snapElement.node);
